...
logfilename = path + f"log/{location}_{startdate}to{enddate}_predictions.log"
...
MPC_bodies = np.arange(1,773000) # use numbers instead of names. 773917 is the max as of right now
print(MPC_bodies[-1])
logging.info(f"Final numbered body is: {MPC_bodies[-1]}")


# define Observer class from location
if location =='VERITAS':
...
elif location == "CTAO-North":
    T1 = Observer(name="CTAO-N", lat="28 45 43.8", lon="-17 53 31.2", height=2200)
elif location == "CTAO-South":
    T1 = Observer(name="CTAO-S", lat="-24 41 0.3", lon="-70 18 58.8", height=2082)
...
for n in MPC_bodies:
    ...
    logging.info(f"MPC Body {asteroid.shortname}")
    print(f"Occultation Found Boolean is: {occultation_found}")
    if occultation_found:
        new_pred = prediction(body=asteroid, time_beg=startdate,time_end=enddate,mag_lim=15.0, reference_center=T1)
        if new_pred is not None and len(new_pred) > 0:
        ...
        pred = prediction(body=asteroid, time_beg=startdate,time_end=enddate,mag_lim=15.0, reference_center=T1)
        print(pred)
        logging.info(pred)
...
ascii.write(pred, f'{path}/predictions/{location}_{startdate}to{enddate}_predictions.dat', overwrite=True) 
